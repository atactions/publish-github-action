...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (!check_tag) {
                yield exec.exec('git', ['branch', '-d', branchName], { ignoreReturnCode: true }).catch(f);
                console.log('delete branch ', branchName);
                yield exec.exec('git', ['tag', '-d', version], { ignoreReturnCode: true }).catch(f);
                console.log('delete tag ', version);
                yield exec.exec('git', ['tag', '-d', majorVersion], { ignoreReturnCode: true }).catch(f);
                console.log('delete tag ', majorVersion);
                yield exec.exec('git', ['tag', '-d', minorVersion], { ignoreReturnCode: true }).catch(f);
                console.log('delete tag ', minorVersion);
            }
            var f = (value) => { console.log('delete refs:', value); };
            yield exec.exec('git', ['checkout', 'master']);
            yield exec.exec('git', ['checkout', '-b', branchName], { ignoreReturnCode: true }).catch(f);
            ...
            yield exec.exec('git', ['push', 'origin', ':' + branchName], { ignoreReturnCode: true }).catch(f);
            yield exec.exec('git', ['push', 'origin', branchName]);
            yield exec.exec('git', ['push', 'origin', ':refs/tags/' + version], { ignoreReturnCode: true }).catch(f);
            yield exec.exec('git', ['tag', '-fa', version, '-m', version]);
            yield exec.exec('git', ['push', 'origin', ':refs/tags/' + minorVersion], { ignoreReturnCode: true }).catch(f);
            yield exec.exec('git', ['tag', '-f', minorVersion]);
            yield exec.exec('git', ['push', 'origin', ':refs/tags/' + majorVersion], { ignoreReturnCode: true }).catch(f);
